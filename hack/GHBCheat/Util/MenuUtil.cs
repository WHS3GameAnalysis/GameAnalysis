...
namespace GHBCheat.Util
{
    ...
    public class MenuUtil
    {
        ...
        {
            GHBCheat.localPlayer?.playerActions.Disable();
            Cursor.visible = true;
        ...
        {
            if (GHBCheat.localPlayer == null)
            {
                if (!Cursor.visible || Cursor.lockState != CursorLockMode.None) ShowCursor();
                return;
            }
            GHBCheat.localPlayer?.playerActions.Enable();
        ...
        //{
        //    if (LMUserCoroutine == null) LMUserCoroutine = GHBCheat.Instance.StartCoroutine(LMUser());
        //    else SendLMUserMessage = true;
        //}

        //private static IEnumerator LMUser()
        //{
        //    while (!Settings.b_DisplayLMUsers || HUDManager.Instance == null || GHBCheat.localPlayer == null) yield return new WaitForSeconds(10f);
        //    HUDManager.Instance.Reflect().Invoke("AddTextMessageServerRpc", $"<size=0>{GHBCheat.localPlayer.playerSteamId}, {Settings.version}</size>");
        ...
        //        {
        //            if (s.Match(m).Success && v.Match(m).Success) GHBCheat.Instance.LMUsers[s.Match(m).Value] = v.Match(m).Groups[1].Value;
        //        });
